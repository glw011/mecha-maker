...
void ApplLangInterp::assignVar(const std::string& name, const ApplValue& val) {
    // Update existing binding in the nearest enclosing scope that owns it.
    for (int i = static_cast<int>(scopeStack_.size()) - 1; i >= 0; --i) {
        auto it = scopeStack_[i].find(name);
        if (it != scopeStack_[i].end()) { it->second = val; return; }
    }
    // New binding goes in the innermost frame.
...
bool ApplLangInterp::hasVar(const std::string& name) const {
    for (int i = static_cast<int>(scopeStack_.size()) - 1; i >= 0; --i)
        if (scopeStack_[i].count(name)) return true;
    return false;
}
...
ApplValue ApplLangInterp::valueFrom(const std::any& a) {
    ...
    try { return std::any_cast<ApplValue>(a); }
    catch (const std::bad_any_cast&) {
        throw ApplRuntimeError("Internal: unexpected value type in expression");
    }
}
...
// ---------------------------------------------------------------------------
//  Arithmetic — result type follows C promotion: either operand FLT → FLT.
// ---------------------------------------------------------------------------
ApplValue ApplLangInterp::applyArith(const std::string& op,
                                      const ApplValue& lhs,
                                      const ApplValue& rhs) {
    if (op == "//" || op == "%") {
        int l = lhs.toInt(), r = rhs.toInt();
        if (r == 0) throw ApplRuntimeError("Division by zero");
        return ApplValue(op == "//" ? l / r : l % r);
    }
    bool useFloat = lhs.isFlt() || rhs.isFlt();
    if (op == "+") return useFloat ? ApplValue(lhs.toDouble() + rhs.toDouble()) : ApplValue(lhs.toInt() + rhs.toInt());
    if (op == "-") return useFloat ? ApplValue(lhs.toDouble() - rhs.toDouble()) : ApplValue(lhs.toInt() - rhs.toInt());
    if (op == "*") return useFloat ? ApplValue(lhs.toDouble() * rhs.toDouble()) : ApplValue(lhs.toInt() * rhs.toInt());
    if (op == "/") {
        if (!useFloat && rhs.toInt()    == 0)   throw ApplRuntimeError("Division by zero");
        if ( useFloat && rhs.toDouble() == 0.0) throw ApplRuntimeError("Division by zero");
        return useFloat ? ApplValue(lhs.toDouble() / rhs.toDouble())
                        : ApplValue(lhs.toInt()    / rhs.toInt());
    }
    if (op == "**") return ApplValue(std::pow(lhs.toDouble(), rhs.toDouble()));
    throw ApplRuntimeError("Unknown arithmetic operator: '" + op + "'");
}

ApplValue ApplLangInterp::applyCompareFn(const std::string& op,
                                          const ApplValue& lhs,
                                          const ApplValue& rhs) {
    if (lhs.isBool() && rhs.isBool()) {
        if (op == "==") return ApplValue(lhs.bval == rhs.bval);
        if (op == "!=") return ApplValue(lhs.bval != rhs.bval);
        throw ApplRuntimeError("Operator '" + op + "' not valid for booleans");
    }
    double l = lhs.toDouble(), r = rhs.toDouble();
    if (op == "==") return ApplValue(l == r);
    if (op == "!=") return ApplValue(l != r);
    if (op == "<")  return ApplValue(l <  r);
    if (op == "<=") return ApplValue(l <= r);
    if (op == ">")  return ApplValue(l >  r);
...
ApplValue ApplLangInterp::applyAugop(const std::string& op,
                                      const ApplValue& current,
                                      const ApplValue& rhs) {
    if (op == "+=")  return applyArith("+",  current, rhs);
    if (op == "-=")  return applyArith("-",  current, rhs);
    if (op == "*=")  return applyArith("*",  current, rhs);
    if (op == "/=")  return applyArith("/",  current, rhs);
    if (op == "//=") return applyArith("//", current, rhs);
...
// ---------------------------------------------------------------------------
//  Operator token extraction helper.
//
//  ANTLR context methods like ctx->ADD(i) return the i-th terminal of that
//  token type. The correct null-check is simply: token != nullptr.
//  TerminalNode has no isEmpty() — that was the bug in the original code.
//
//  For rules with multiple interleaved operator types (comparison, mult) we
//  walk the raw child list to read each operator token in source order.
//  Children alternate: operand  op  operand  op  operand ...
//  So operator children sit at odd indices (1, 3, 5, ...).
// ---------------------------------------------------------------------------
static std::string childOpText(antlr4::ParserRuleContext* ctx, size_t opChildIndex) {
    // opChildIndex is the 0-based index into ctx->children.
    
    if (opChildIndex < ctx->children.size()) {
        auto* node = dynamic_cast<antlr4::tree::TerminalNode*>(
                         ctx->children[opChildIndex]);
...
std::any ApplLangInterp::visitMain(APPL_Parse::MainContext* ctx) {
    ...
    for (auto* stmt : ctx->statement())
        if (stmt->fun_def()) visitFun_def(stmt->fun_def());

    // Second pass: execute non-def statements.
    try {
        for (auto* stmt : ctx->statement())
            if (!stmt->fun_def()) visitStatement(stmt);
    } catch (const ApplReturnException&) {
        // Top-level return is silently swallowed.
    }
...
std::any ApplLangInterp::visitStatement(APPL_Parse::StatementContext* ctx) {
    if      (ctx->assign())       visitAssign(ctx->assign());
    else if (ctx->fun_def())      visitFun_def(ctx->fun_def());
    else if (ctx->conditional())  visitConditional(ctx->conditional());
    else if (ctx->expr())         visitExpr(ctx->expr());
    else if (ctx->return_())      visitReturn(ctx->return_());
    return {};
}
...
std::any ApplLangInterp::visitConditional(APPL_Parse::ConditionalContext* ctx) {
    if      (ctx->if_else_stmt())  visitIf_else_stmt(ctx->if_else_stmt());
    else if (ctx->while_loop())    visitWhile_loop(ctx->while_loop());
    else if (ctx->for_loop())      visitFor_loop(ctx->for_loop());
    else if (ctx->for_ea_loop())   visitFor_ea_loop(ctx->for_ea_loop());
    return {};
}
...
std::any ApplLangInterp::visitFor_ea_loop(APPL_Parse::For_ea_loopContext* ctx) {
    throw ApplRuntimeError(
        "for each loop over '" + ctx->ID(1)->getText() +
...
std::any ApplLangInterp::visitExpr(APPL_Parse::ExprContext* ctx) {
    if (ctx->operatn()) return visitOperatn(ctx->operatn());
    return visitFactor(ctx->factor());
}

// operatn : unary_oprtn | binary_oprtn
std::any ApplLangInterp::visitOperatn(APPL_Parse::OperatnContext* ctx) {
    if (ctx->unary_oprtn()) return visitUnary_oprtn(ctx->unary_oprtn());
...
std::any ApplLangInterp::visitUnary_oprtn(APPL_Parse::Unary_oprtnContext* ctx) {
    if (ctx->iteratn()) return visitIteratn(ctx->iteratn());
    return visitPrefx_unary_oprtn(ctx->prefx_unary_oprtn());
}

// iteratn : factor INCR | factor DECR   (post-increment / post-decrement)
std::any ApplLangInterp::visitIteratn(APPL_Parse::IteratnContext* ctx) {
    if (!ctx->factor()->ID())
        throw ApplRuntimeError("Increment/decrement requires a variable operand");

    const std::string name = ctx->factor()->ID()->getText();
    ApplValue current = lookupVar(name);
    ApplValue result  = current;  // return pre-mutation value

    if (ctx->INCR())
        assignVar(name, current.isInt() ? ApplValue(current.ival + 1) : ApplValue(current.dval + 1.0));
    else
        assignVar(name, current.isInt() ? ApplValue(current.ival - 1) : ApplValue(current.dval - 1.0));
...
std::any ApplLangInterp::visitPrefx_unary_oprtn(APPL_Parse::Prefx_unary_oprtnContext* ctx) {
    if (ctx->negatn()) return visitNegatn(ctx->negatn());

    if (!ctx->factor()->ID())
        throw ApplRuntimeError("Prefix increment/decrement requires a variable operand");

    const std::string name = ctx->factor()->ID()->getText();
    ApplValue current = lookupVar(name);
    ApplValue updated = ctx->INCR()
        ? (current.isInt() ? ApplValue(current.ival + 1) : ApplValue(current.dval + 1.0))
        : (current.isInt() ? ApplValue(current.ival - 1) : ApplValue(current.dval - 1.0));

    assignVar(name, updated);
    return updated;  // return post-mutation value
}
...
std::any ApplLangInterp::visitBinary_oprtn(APPL_Parse::Binary_oprtnContext* ctx) {
    if (ctx->logic_or_expr()) return visitLogic_or_expr(ctx->logic_or_expr());
...
// Short-circuit: stop as soon as a clause is true.
std::any ApplLangInterp::visitLogic_or_expr(APPL_Parse::Logic_or_exprContext* ctx) {
    // Fix: const auto& (not auto&) to bind to the returned temporary vector.
...
//
//  Operator detection fix: TerminalNode has no isEmpty(). Instead we walk the
//  raw child list. Children alternate operand/operator:
//    child[0] = add_expr,  child[1] = op token,  child[2] = add_expr, ...
//  So operator i sits at child index  (2*i + 1).
// ---------------------------------------------------------------------------
...
// pow_expr : factor | factor POW pow_expr   (right-associative)
...
std::any ApplLangInterp::visitFun_call(APPL_Parse::Fun_callContext* ctx) {
    ...
    if (it != functionTable_.end()) {
        ...
        if (args.size() != def.params.size())
            throw ApplRuntimeError(
                "Function '" + name + "' expects " + std::to_string(def.params.size()) +
    ...
    throw ApplRuntimeError("Undefined function: '" + name + "'");
}
