...
std::unordered_map<CodeBlocks::BlockType, int> CodeBlocks::SlotCountMap = {
  {BlockType::NONE,           0},
  {BlockType::MAIN,           1},
  {BlockType::ASSIGN_BLK,     2},  // slot0=VarRef, slot1=expr
  ...
  {BlockType::WHILE_LOOP,     2},  // slot0=cond, slot1=cblk
  {BlockType::FOR_LOOP,       4},  // slot0=init(assign), slot1=cond, slot2=iter(unaryOp), slot3=cblk
  ...
  {BlockType::AUG_ASSIGN_BLK, 2},  // slot0=VarRef, slot1=expr  (augOp in userInputVals[0])
};
...
std::unordered_map<CodeBlocks::BlockType, std::string> CodeBlocks::ContentStrMap = {
  ...
  {BlockType::FOR_LOOP,       "for ({initSlot}{condSlot}{iterSlot}){cblkSlot}"},
  ...
  {BlockType::ELSE_SBLK,      "else {cblkSlot}"},
